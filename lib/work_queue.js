...
var WorkQueue = function WorkQueue(queueName, eventBus, queueManager){
...
function bindWorkerQueues(existingBindings, callback) {
  ...
  // Receive messages
  this.workQueue.subscribe({ack: true, prefetchCount: 20}, function (message, headers, deliveryInfo, messageObject) {
    ...
    this.eventBus.trigger(workerId, message, function(err){
      if (err) {
