  ...
  rest.get(url, this.httpOptions)
    .on('complete', function(result, response){
      if (result instanceof Error) {
        ...
        var bindings = JSON.parse(response.raw);
...
function bindWorkerQueues(existingBindings, callback) {
  ...
  // Receive messages
  this.workQueue.subscribe({ack: true, prefetchCount: 5}, function (message, headers, deliveryInfo, messageObject) {
...
function createQueue(params, undefined, done){
  ...
  this[assignToProp] = this.connection.queue(queueName , {
    ...
    done(new Error('unable to create queue ' + queueName));
  });
}
...
function retryOrDiscard(message, messageObject, deliveryInfo){
  ...
  var delayFunc = Job.getBackoffImpl({
    type: headers.backoff,
    delay:headers.delay
  });
  ...
  var messageOptions = {
    ...
    headers: {
      ...
      delay: headers.delay,
      max_attempts: headers.max_attempts,
      made_attempts: madeAttempts,
...
WorkQueue.prototype.discardInvalidWorkMsg = function discardInvalidWorkMsg(msg){
  logger.warn("WorkerQueue '" + this.name + "' have received an invalid job that doesn't have a workId ");
};
